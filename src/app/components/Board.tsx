...
export default function Board() {
    ...
    function mu(e: MouseEvent) {
      ...
      if (dragCardId.current && boardRef.current) {
        ...
        if (e.clientX >= r.left && e.clientX <= r.right && e.clientY >= r.top && e.clientY <= r.bottom) {
          const x = Math.max(10, e.clientX - r.left - 82)
          const y = Math.max(10, e.clientY - r.top  - 55)
          dropCard(dragCardId.current, x, y)
        }
      }
  ...
  function dropCard(cardId: string, x: number, y: number) {
    ...
    setBoardCards(prev => ({
      ...prev,
      [cardId]: { ...src, x, y, note: '', status: 'witness' as StatusType }
    }))
    setMovesUsed(m => m + 1)
  }
  ...
  function startConnect(id: string) {
    ...
    const types = STRING_TYPES[card?.type] || []
  ...
  const availableStringTypes = connectingFrom
    ? STRING_TYPES[boardCards[connectingFrom]?.type] || []
    : []
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', background: '#0a0805' }}>
          ...
          >
              ...
              {strings.map((s, i) => {
                ...
                return (
                  <path key={i}
                    ...
                    fill="none"
                    stroke={color}
                    strokeWidth={s.type === 'suspects' || s.type === 'was-there' ? 3 : 2.5}
                    strokeLinecap="round"
            ...
            {connectingFrom && (
              <div style={{
                ...
                <div style={{ fontFamily: 'Special Elite, cursive', color: 'var(--gold)', fontSize: 11, letterSpacing: 2 }}>
            ...
            <div style={{
              ...
              background: 'rgba(8,4,0,.96)', border: '1px solid #6a5a3a',
              padding: '10px 14px', zIndex: 50,
            }}>
              <div style={{ fontFamily: 'Special Elite, cursive', color: 'var(--gold)', fontSize: 10, letterSpacing: 2, marginBottom: 7, borderBottom: '1px solid #3a2a10', paddingBottom: 5 }}>
                Strings
              </div>
              {Object.entries(STRING_LABELS).map(([key, label]) => (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 5 }}>
                  <div style={{
                    width: 28, height: 2.5, flexShrink: 0, borderRadius: 1,
                    background: STRING_COLORS[key],
                    boxShadow: `0 0 4px ${STRING_COLORS[key]}88`,
                  }} />
                  <div style={{ fontSize: 10, color: '#c8b88a', letterSpacing: 1, textTransform: 'uppercase' }}>
                    {label}
                  </div>
                </div>
              ))}
      ...
      {/* Status bar */}
      <div style={{
        background: 'var(--sidebar)', borderTop: '1px solid var(--sidebar-border)',
        padding: '4px 16px', display: 'flex', gap: 20, flexShrink: 0,
      }}>
        {[
        ...
        >
          <div style={{ background: 'var(--paper)', border: '2px solid var(--paper-dark)', maxWidth: 400, width: '90%', boxShadow: '4px 8px 30px rgba(0,0,0,.7)' }}>
            ...
            <div style={{ padding: '12px 14px' }}>
              <div style={{ fontSize: 11, color: 'var(--ink-faded)', lineHeight: 1.7, fontStyle: 'italic', marginBottom: 10 }}>
                {modal.card.desc}
              </div>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    {(['witness','suspect','poi','alibi'] as StatusType[]).map(s => (
                      <button key={s} onClick={() => {
                        ...
                        fontFamily: 'Courier Prime, monospace', letterSpacing: 1, textTransform: 'uppercase',
                      }}>
                        {s}
                      </button>
                    ))}
      ...
      {doc && (
        <div style={{
          position: 'fixed', top: 0, left: 0, width: '100%', height: '100%',
          background: 'rgba(3,2,0,.92)', zIndex: 400,
          ...
          <div style={{ background: '#0d0a06', borderBottom: '1px solid #2a1f0a', padding: '8px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => setDoc(null)} style={{ ...topBtnStyle, borderColor: 'var(--red)', color: '#cc2222' }}>✕ Close</button>
          </div>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#2a2015' }}>
            <div style={{ width: 360, background: 'var(--paper)', border: '1px solid var(--paper-dark)', padding: 30, boxShadow: '0 4px 20px rgba(0,0,0,.5)' }}>
              <div style={{ fontFamily: 'Special Elite, cursive', fontSize: 8, letterSpacing: 3, color: '#5a4a2a', textTransform: 'uppercase', border: '2px solid #5a4a2a', padding: '3px 8px', display: 'inline-block', transform: 'rotate(-2deg)', marginBottom: 14 }}>
                Evidence document
              </div>
              <div style={{ fontFamily: 'Special Elite, cursive', fontSize: 16, color: 'var(--ink)', marginBottom: 10, borderBottom: '1px solid var(--paper-dark)', paddingBottom: 8 }}>
                {doc.title}
              </div>
              <div style={{ fontSize: 10, color: 'var(--ink-faded)', lineHeight: 1.8, whiteSpace: 'pre-line' }}>
                {doc.content}
              </div>
            </div>
          </div>
